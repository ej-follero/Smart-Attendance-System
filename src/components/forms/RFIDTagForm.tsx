...
export function RFIDTagForm({ 
  ...
  const { setMode, isConnected, recentScans, sendFeedback } = useMQTTRFIDScan({
    ...
    onNewScan: (scan) => {
      ...
      console.log('[RFID Tag Form] MQTT Connected:', isConnected, 'Recent scans:', recentScans.length);
      
      // Validate scan data - must have valid RFID data and timestamp
      if (!scan || !scan.rfid || !scan.timestamp) {
        console.log('[RFID Tag Form] Invalid scan data, ignoring');
        return;
      }
      ...
      if (timeDiff > 5000) { // 5 seconds
        console.log('[RFID Tag Form] Scan too old, ignoring');
        return;
      }
      
      // Only process scans that occurred after this component was mounted
      if (scanTime < componentMountTime) {
        console.log('[RFID Tag Form] Scan occurred before component mount, ignoring');
        return;
      }
      
      const tag = String(scan.rfid).trim();
      if (!tag) {
        console.log('[RFID Tag Form] Empty tag number, ignoring');
        return;
      }
      ...
      if (lastProcessedScan === scanId) {
        console.log('[RFID Tag Form] Scan already processed, ignoring');
        return;
      }
      
      console.log('[RFID Tag Form] Processing new RFID scan:', tag);
