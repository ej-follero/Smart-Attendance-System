...
export default function LiveAttendanceFeed() {
  ...
  useEffect(() => {
    if (mqttClient?.messages && mqttClient.messages.length > 0) {
      ...
      if (messageId !== lastProcessedMessageId && 
          ...
          !isRefreshing) {
        console.log('[LiveAttendance] New MQTT attendance message received:', latestMessage);
        setLastProcessedMessageId(messageId);
        setIsRefreshing(true);
        
        console.log('[LiveAttendance] Refreshing live attendance data due to MQTT scan');
  ...
  const fetchData = async () => {
    try {
      ...
      console.log('[LiveAttendance] DEBUG: Fetching live attendance data from API...');
      console.log('[LiveAttendance] DEBUG: Current day filter:', today);
      console.log('[LiveAttendance] DEBUG: API URL:', `/api/attendance/live-feed?${params}`);
      console.log('[LiveAttendance] DEBUG: Full URL:', `${window.location.origin}/api/attendance/live-feed?${params}`);
      ...
      if (result.success) {
        ...
        if (FORCE_EMPTY_STATE) {
          console.log('[LiveAttendance] FORCE_EMPTY_STATE is enabled - showing empty state');
          data = {
            records: [],
            pagination: { total: 0, limit: 25, offset: 0, hasMore: false },
            statistics: { totalRecords: 0, statusCounts: {} },
  ...
  useEffect(() => {
    ...
    eventSource.onmessage = (event) => {
      try {
        ...
        console.log('[LiveAttendance] Real-time stream data received:', eventData);
        
        if (eventData.type === 'attendance_update' && eventData.records) {
          console.log('[LiveAttendance] New attendance records:', eventData.records);
          ...
          setData(prevData => {
            ...
            console.log('[LiveAttendance] Adding new records to table:', newRecords.length);
            return {
              ...prevData,
          ...
          <QuickActionsPanel
            ...
            actionCards={[
              ...
              },
              {
                ...
                icon: <RefreshCw className="w-5 h-5 text-white" />,
                onClick: () => {
                  console.log('[LiveAttendance] Manual refresh triggered');
                  fetchData();
                }
              },
              {
