...
export default function DepartmentListPage() {
  ...
  const convertImageToBase64 = (imageUrl: string): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      console.log('[Departments] Converting image URL:', fullUrl);
      ...
      img.onload = () => {
        try {
          console.log('[Departments] Image loaded successfully, dimensions:', img.width, 'x', img.height);
          const canvas = document.createElement('canvas');
          const ctx = canvas.getContext('2d');
          if (!ctx) {
            reject(new Error('Could not get canvas context'));
          ...
          console.log('[Departments] Successfully converted to base64, length:', dataURL.length);
          console.log('[Departments] Base64 data starts with:', dataURL.substring(0, 50));
          console.log('[Departments] Image format:', dataURL.startsWith('data:image/jpeg') ? 'JPEG' : 'PNG');
          resolve(dataURL);
        } catch (error) {
          console.error('[Departments] Error converting image to base64:', error);
          reject(error);
        }
      };
      
      img.onerror = (error) => {
        console.error('[Departments] Error loading image with CORS:', error);
        console.error('[Departments] Trying without CORS...');
        
        // Fallback: try without CORS
        const imgNoCors = new window.Image();
        imgNoCors.onload = () => {
          try {
            console.log('[Departments] Image loaded without CORS, dimensions:', imgNoCors.width, 'x', imgNoCors.height);
            const canvas = document.createElement('canvas');
            const ctx = canvas.getContext('2d');
            if (!ctx) {
              reject(new Error('Could not get canvas context'));
            ...
            console.log('[Departments] Successfully converted to base64 (no CORS), length:', dataURL.length);
            resolve(dataURL);
          } catch (error) {
            console.error('[Departments] Error converting image to base64 (no CORS):', error);
            reject(error);
          }
        };
        
        imgNoCors.onerror = () => {
          console.error('[Departments] Failed to load image even without CORS:', fullUrl);
        ...
        case 'pdf':
          ...
          if (includeLogos) {
            ...
            const logoPromises = filteredDepartments.map(async (dept, index) => {
              ...
              console.log(`[Departments] Department ${dept.name}: logo field =`, dept.logo);
              console.log(`[Departments] Department ${dept.name}: logo type =`, typeof dept.logo);
              
              if (dept.logo) {
                console.log(`Processing logo for ${dept.name}:`, dept.logo);
                try {
                  logoData = await convertImageToBase64(dept.logo);
                  console.log(`[Departments] Successfully converted logo for ${dept.name}, data length:`, logoData.length);
                  console.log(`Logo data starts with data:image:`, logoData.startsWith('data:image'));
                  
                  if (!logoData.startsWith('data:image')) {
                    console.error(`[Departments] Invalid image data for ${dept.name}:`, logoData.substring(0, 100));
                    throw new Error('Invalid image data');
                  }
                  
                  // Make the logo rounded
                  logoData = await makeImageRounded(logoData);
                  console.log(`[Departments] Made logo rounded for ${dept.name}`);
                } catch (error) {
                  console.warn(`[Departments] Failed to convert logo for department ${dept.name}:`, error);
                  // Don't re-throw, create default avatar instead
                  console.log(`[Departments] Creating default avatar for ${dept.name} due to logo conversion failure`);
                  logoData = createDefaultAvatar();
                }
              } else {
                console.log(`No logo for ${dept.name}, creating default avatar`);
                logoData = createDefaultAvatar();
                console.log(`[Departments] Created default avatar for ${dept.name}`);
              }
            ...
            console.log(`[Departments] Logo conversion results:`, logoResults.length);
            
            // Store logo data in map
            logoResults.forEach(({ index, logoData, deptName }) => {
              logoDataMap.set(index, logoData);
              console.log(`[Departments] Stored logo data for ${deptName} at index ${index}, data length: ${logoData.length}`);
              console.log(`[Departments] Logo data preview:`, logoData.substring(0, 100) + '...');
            });
            
            console.log(`[Departments] Logo processing completed. Total logos: ${logoDataMap.size}`);
            ...
            tableRows = filteredDepartments.map((dept, index) => {
              ...
              console.log(`[Departments] Row ${index} for ${dept.name}:`, newRow);
              console.log(`[Departments] Row ${index} logoDataMap has key:`, logoDataMap.has(index));
              return newRow;
            });
          }
          ...
          logoDataMap.forEach((logoData, index) => {
            console.log(`[Departments] LogoDataMap[${index}]: length=${logoData.length}, startsWith=data:image: ${logoData.startsWith('data:image')}`);
          });
          ...
          const tableResult = autoTable(doc, {
            ...
            didParseCell: includeLogos ? (data) => {
              console.log(`[Departments] didParseCell called: column=${data.column.index}, row=${data.row.index}, section=${data.cell.section}, text="${data.cell.text[0]}"`);
              // Handle logo images in the first column (logo column) - but NOT the header row
              if (data.column.index === 0 && data.cell.section === 'body') {
                const cellText = data.cell.text[0];
                console.log(`[Departments] didParseCell: Logo column cell detected: "${cellText}" at row ${data.row.index}`);
                ...
                if (cellText === 'LOGO_PLACEHOLDER') {
                  ...
                  console.log(`[Departments] Looking for logo data at index ${data.row.index}, found:`, !!logoData);
                  console.log(`[Departments] Total logoDataMap size:`, logoDataMap.size);
                  console.log(`[Departments] Available keys:`, Array.from(logoDataMap.keys()));
                  ...
                  data.cell.text = [''];
                  console.log(`[Departments] Cleared cell text for row ${data.row.index}`);
                } else {
                  console.log(`[Departments] Cell text is not 'LOGO_PLACEHOLDER': "${cellText}"`);
                }
              } else if (data.column.index === 0 && data.cell.section === 'head') {
                console.log(`[Departments] Header row detected, keeping text: "${data.cell.text[0]}"`);
              }
            } : undefined,
            didDrawCell: includeLogos ? (data) => {
              console.log(`[Departments] didDrawCell called: column=${data.column.index}, row=${data.row.index}, section=${data.cell.section}, text="${data.cell.text[0]}"`);
              // Handle logo images in the first column (logo column) - but NOT the header row
              if (data.column.index === 0 && data.cell.section === 'body') {
                const cellText = data.cell.text[0];
                console.log(`[Departments] didDrawCell: Logo column cell detected: "${cellText}" at row ${data.row.index}`);
                ...
                if (cellText === '' || cellText === 'LOGO_PLACEHOLDER') {
                  ...
                  if (logoData) {
                    try {
                      ...
                      console.log(`[Departments] Cell position: x=${data.cell.x}, y=${data.cell.y}, width=${data.cell.width}, height=${data.cell.height}`);
                      console.log(`[Departments] Image position: x=${x}, y=${y}, size=${imgWidth}x${imgHeight}`);
                      console.log(`[Departments] Logo data length:`, logoData.length);
                      console.log(`[Departments] Image format:`, imageFormat);
                      console.log(`[Departments] Logo data preview:`, logoData.substring(0, 100) + '...');
                      
                      // Add the image to the PDF with correct format
                      doc.addImage(logoData, imageFormat, x, y, imgWidth, imgHeight);
                      console.log(`[Departments] Successfully added logo for row ${data.row.index}`);
                    } catch (error) {
                      console.error(`[Departments] Error adding logo for row ${data.row.index}:`, error);
                      console.error(`[Departments] Error details:`, error instanceof Error ? error.message : String(error));
                      // Don't set text back, leave it empty
                    }
                  } else {
                    console.log(`[Departments] No logo data found for row ${data.row.index}`);
                    // Don't set text back, leave it empty
                  }
                }
              } else if (data.column.index === 0 && data.cell.section === 'head') {
                ...
                <BulkActionsBar
                  ...
                  actions={[
                    ...
                    },
                    {
                      ...
                      icon: pageState.loading ? <Loader2 className="h-4 mr-2 animate-spin" /> : <RotateCcw className="w-4 h-4 mr-2" />,
