...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    for (const g of grouped) {
      ...
      if (!sid) continue;
      const count = typeof g._count === 'object' && g._count !== null && '_all' in g._count 
        ? (g._count as { _all: number })._all 
        : 0;
      const c = perSched.get(sid) ?? { present: 0, absent: 0, late: 0, excused: 0, total: 0 };
      switch (g.status) {
        case 'PRESENT':
          c.present += count; break;
        case 'ABSENT':
          c.absent += count; break;
        case 'LATE':
          c.late += count; break;
        case 'EXCUSED':
          c.excused += count; break;
        default:
    ...
    if (process.env.NODE_ENV === 'development' && searchParams.get('debug') === '1' && e instanceof Error) {
