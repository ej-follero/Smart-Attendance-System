...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingReader) {
      console.log('[MQTT] Reader already exists:', deviceId);
      return NextResponse.json({ 
        success: false, 
    ...
    console.log('[MQTT] New reader registered:', newReader.readerId);
    ...
    return NextResponse.json({ 
      success: false, 
      error: process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Failed to register reader'
    }, { status: 500 });
  }
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[MQTT] Get unassigned readers error:', error);
    return NextResponse.json({ 
      success: false, 
      error: process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Failed to get unassigned readers'
    }, { status: 500 });
  }
}
