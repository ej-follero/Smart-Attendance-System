...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json({ 
      error: process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Failed to update role'
    }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ 
      error: process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Failed to delete role'
    }, { status: 500 });
  }
}