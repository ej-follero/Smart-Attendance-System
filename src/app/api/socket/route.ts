...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: false,
      error: process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Failed to get Socket.IO status'
    }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: false,
      error: process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Failed to broadcast message'
    }, { status: 500 });
  }
}
