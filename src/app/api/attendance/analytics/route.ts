...
export async function GET(request: NextRequest) {
  ...
  console.log('Sample attendance records:', attendanceRecords.slice(0, 5).map(r => ({
    ...
    if (attendanceRecords.length > 0) {
      ...
      console.log(`[Analytics] Unique dates in attendance records: ${uniqueTimestamps.size}`);
      console.log(`[Analytics] Date range: ${Math.min(...timestamps.map(t => t.getTime()))} to ${Math.max(...timestamps.map(t => t.getTime()))}`);
      ...
      console.log(`[Analytics] Month distribution:`, monthCounts);
      ...
      console.log(`[Analytics] Status distribution:`, statusCounts);
    }
    ...
    console.log(`[Analytics] Building summary from ${attendanceRecords.length} filtered records`);
    console.log(`[Analytics] Applied filters: departmentId=${departmentId}, courseId=${courseId}, sectionId=${sectionId}, yearLevel=${yearLevel}, subjectId=${subjectId}, timeRange=${timeRange}`);
    ...
    console.log(`[Analytics] Summary counts: Present=${presentCount}, Late=${lateCount}, Absent=${absentCount}, Total=${totalAttendance}`);
    console.log(`[Analytics] Attendance rate: ${attendanceRateSummary.toFixed(1)}%`);
...
function processTimeBasedData(records: any[], timeRange: string, dateStart: Date, dateEnd: Date) {
  ...
  console.log(`[Analytics] processTimeBasedData - Processing ${records.length} records for ${timeRange}`);
  console.log(`[Analytics] Date range: ${dateStart.toISOString()} to ${dateEnd.toISOString()}`);
  console.log(`[Analytics] processTimeBasedData - These records are already filtered by department, course, section, year level, subject, and time range`);
  ...
  console.log(`[Analytics] processTimeBasedData - Generated ${result.length} data points for ${timeRange}:`, result);
  console.log(`[Analytics] processTimeBasedData - After filling missing points: ${filledResult.length} data points`);
  console.log(`[Analytics] processTimeBasedData - DataMap entries:`, Array.from(dataMap.entries()));
  ...
  if (filledResult.length > 0) {
    ...
    console.log(`[Analytics] Unique attendance rates: ${uniqueRates.size} (${Array.from(uniqueRates).join(', ')})`);
    
    if (uniqueRates.size === 1) {
      console.log(`WARNING: All data points have the same attendance rate (${Array.from(uniqueRates)[0]}%)`);
      console.log(`[Analytics] This will result in flat chart lines. Check if attendance records have proper time variation.`);
    }
  }
...
function processDepartmentStats(records: any[]) {
  ...
  console.log(`[Analytics] Processing ${records.length} records for department stats`);
  ...
  console.log(`[Analytics] Department stats: ${processedCount} processed, ${skippedCount} skipped`);
  console.log(`[Analytics] Found ${deptMap.size} departments:`, Array.from(deptMap.keys()));
  
  const result = Array.from(deptMap.values());
  console.log(`[Analytics] Department stats result:`, result);
...
function processLateArrivalData(records: any[], timeRange: string, dateStart: Date, dateEnd: Date) {
  ...
  console.log(`[Analytics] processLateArrivalData - Processing ${records.length} records for ${timeRange}`);
  console.log(`[Analytics] Date range: ${dateStart.toISOString()} to ${dateEnd.toISOString()}`);
  console.log(`[Analytics] processLateArrivalData - These records are already filtered by department, course, section, year level, subject, and time range`);
  ...
  console.log(`[Analytics] processLateArrivalData - Generated ${result.length} data points for ${timeRange}:`, result);
  console.log(`[Analytics] processLateArrivalData - After filling missing points: ${filledResult.length} data points`);
  console.log(`[Analytics] processLateArrivalData - DataMap entries:`, Array.from(dataMap.entries()));
...
function processPatternAnalysis(records: any[], timeRange: string, dateStart: Date, dateEnd: Date) {
  ...
  console.log(`[Analytics] processPatternAnalysis - Processing ${records.length} records for ${timeRange}`);
  console.log(`[Analytics] Date range: ${dateStart.toISOString()} to ${dateEnd.toISOString()}`);
  console.log(`[Analytics] processPatternAnalysis - These records are already filtered by department, course, section, year level, subject, and time range`);
  ...
  console.log(`[Analytics] processPatternAnalysis - Generated ${result.length} data points for ${timeRange}:`, result);
  console.log(`[Analytics] processPatternAnalysis - After filling missing points: ${filledResult.length} data points`);
...
function processStreakAnalysis(records: any[], timeRange: string, dateStart: Date, dateEnd: Date) {
  console.log(`[Analytics] processStreakAnalysis - Processing ${records.length} records for ${timeRange}`);
  console.log(`[Analytics] Date range: ${dateStart.toISOString()} to ${dateEnd.toISOString()}`);
  console.log(`[Analytics] processStreakAnalysis - These records are already filtered by department, course, section, year level, subject, and time range`);
  ...
  console.log(`[Analytics] processStreakAnalysis - Generated ${filledTimeBasedData.length} data points for ${timeRange}:`, filledTimeBasedData);
  console.log('[Analytics] Streak analysis stats:', stats);
