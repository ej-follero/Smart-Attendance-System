...
export async function GET(req: NextRequest) {
  ...
  try {
    // Build WHERE clause
    const where: any = {};
    // Filter by instructor through SubjectSchedule relation
    if (instructorId) {
    ...
    const itemsPreFilter = rows.map((r) => {
      ...
      const sch = r.subjectSchedId ? scheduleById.get(r.subjectSchedId) : undefined;
      ...
      return {
        ...
        instructorId: sch?.instructorId ?? null,
      };
    });
