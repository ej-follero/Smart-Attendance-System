...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Failed to detect changes'
      },
      { status: 500 }
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { 
        success: false, 
        error: process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Failed to create incremental backup'
      },
      { status: 500 }
    );
  }
