...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const payload: { error: string; details?: string; code?: string; target?: unknown } = { error: 'Failed to update student' };
    if (process.env.NODE_ENV === 'development' && error && typeof error === 'object') {
      const e = error as { message?: string; code?: string; meta?: { target?: unknown } };
      if (typeof e?.message === 'string') payload.details = e.message;
      if (e?.code) payload.code = e.code;
      if (e?.meta?.target !== undefined) payload.target = e.meta.target;
    }
