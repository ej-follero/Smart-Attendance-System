...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      results.tasks.auditLogs.error = process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Task failed';
    ...
    try {
      ...
      results.tasks.reportFiles.errors.push(process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Task failed');
    ...
    try {
      ...
      results.tasks.backupFiles.error = process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Task failed';
    ...
    try {
      ...
      results.tasks.tempFiles.errors.push(process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Task failed');
    ...
    const errorMessage = process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Cleanup failed';
...
async function cleanOldBackupFiles(): Promise<{ deleted: number; error: string | null }> {
  try {
    ...
    const msg = process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Cleanup failed';
