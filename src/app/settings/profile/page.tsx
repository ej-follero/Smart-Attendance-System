  ...
  const handleToggle2FA = async (enabled: boolean) => {
    try {
      if (enabled) {
        ...
        const qrCodeUrl = String(result.data?.qrCodeUrl ?? '');
        const manualKey = String(result.data?.manualEntryKey ?? '');
        const escapeHtmlAttr = (s: string) => s.replace(/&/g, '&amp;').replace(/"/g, '&quot;').replace(/'/g, '&#39;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
        const escapeJsString = (s: string) => s.replace(/\\/g, '\\\\').replace(/'/g, "\\'").replace(/\n/g, '\\n').replace(/\r/g, '\\r');
        const safeQrUrl = qrCodeUrl.startsWith('data:image') || qrCodeUrl.startsWith('https:') ? escapeHtmlAttr(qrCodeUrl) : '';
        const safeManualKeyAttr = escapeHtmlAttr(manualKey);
        const safeManualKeyJs = escapeJsString(manualKey);
          ...
          ">
            ...
            <div style="margin-bottom: 1.5rem;">
              ...
              <div class="qr-code-container">
                <img src="${safeQrUrl}" alt="QR Code" style="width: 200px; height: 200px; display: block;" />
              </div>
            </div>
            ...
            <div class="manual-key-container">
              ...
              <div style="position: relative;">
                ...
                <button 
                  class="copy-btn" 
                  id="copy-key-btn"
                  onclick="navigator.clipboard.writeText('${safeManualKeyJs}').then(() => {
              ...
              modalContent.innerHTML = `
                ...
                ">
                  ...
                  <div style="margin-bottom: 2rem;">
                    ...
                    ">
                      <span style="font-weight: 500;">Warning:</span> <span id="error-text"></span>
                    </div>
                  ...
                  <div style="margin-top: 1.5rem; padding: 1rem; background: #fef3c7; border-radius: 8px; border-left: 4px solid #f59e0b;">
                    <p style="margin: 0; color: #92400e; font-size: 0.875rem; line-height: 1.4;">
                      <strong>Important:</strong> Make sure your device's time is synchronized for the code to work correctly.
                    </p>
                  </div>
                </div>
              `;
      ...
      successModal.innerHTML = `
        ...
        ">
          <div style="margin-bottom: 1.5rem;">
            ...
            ">
              <span style="font-size: 2rem;" aria-hidden="true">OK</span>
            </div>
