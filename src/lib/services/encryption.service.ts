...
export class EncryptionService {
  ...
  private async initializeDefaultKey() {
    ...
    const envPassword = process.env.BACKUP_ENCRYPTION_PASSWORD;
    if (process.env.NODE_ENV === 'production' && !envPassword) {
      throw new Error('BACKUP_ENCRYPTION_PASSWORD is required in production');
    }
    const defaultPassword = envPassword || 'default-backup-password';
