  ...
  static withPerformanceMonitoring<T extends any[], R>(
    ...
    return async (...args: T): Promise<R> => {
      ...
      try {
        ...
        const duration = Date.now() - startTime;
        const errMsg = process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Unknown error';
        const performanceService = new PerformanceService();
        performanceService.recordQuery(query, duration, false, errMsg);
        throw error;
      }
    };
  }
