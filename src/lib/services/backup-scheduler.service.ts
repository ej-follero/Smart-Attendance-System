        ...
        .catch(async (error) => {
          ...
          await db.systemBackup.update({
            ...
            data: {
              ...
              errorMessage: process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Automated backup failed',
              completedAt: new Date(),
            }
          });
          ...
          await db.backupLog.create({
            data: {
              ...
              status: "ERROR",
              message: `Automated backup failed: ${process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Unknown error'}`,
              createdBy: 1,
            }
          });
        });
