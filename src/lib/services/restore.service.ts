...
export class RestoreService {
  ...
  async performRestore(options: RestoreOptions): Promise<RestoreResult> {
    ...
    try {
      ...
      result.validationErrors = [process.env.NODE_ENV === 'development' && error instanceof Error ? error.message : 'Validation failed'];
      return result;
    }
  }
  ...
  async validateRestore(backupId: string): Promise<{
    ...
    try {
      ...
      try {
        ...
        errors.push(`Backup file is corrupted: ${msg}`);
      }
