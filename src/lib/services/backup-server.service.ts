...
class BackupServerService {
  ...
  async performBackup(backupId: string, backupData: any): Promise<{ status: string; size: string; filePath: string; isEncrypted?: boolean }> {
    try {
      ...
      if (error instanceof Error) {
        console.error('Error details:', { message: error.message, backupId });
      }
      throw error;
    }
  }
  ...
  private async exportDatabase(outputPath: string): Promise<void> {
    ...
    try {
      ...
      // Pass password via env so it is not visible in process listing or logs
      const command = `pg_dump -h ${host} -p ${port} -U ${username} -d ${database} -f "${outputPath}" --no-password`;
